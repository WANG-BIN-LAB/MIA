...
def init_dataloader(cfg: DictConfig,
    ...
    """
    Initialize standard train/val/test dataloaders with random split
    Args:
        cfg: Hydra configuration object
        final_timeseires: Time series feature tensor
        final_pearson: Pearson correlation feature tensor
        labels: Ground truth labels
        stratified: Stratified array for balanced class distribution
    Returns:
        List of train, validation, test dataloaders
    """
    ...
    # Update training steps in configuration
    with open_dict(cfg):
...
def init_stratified_dataloader(cfg: DictConfig,
                               ...
                               stratified: np.array) -> List[utils.DataLoader]:
    """
    Initialize dataloaders with stratified shuffle split to maintain class balance
    Args:
        cfg: Hydra configuration object
        final_timeseires: Time series feature tensor
        final_pearson: Pearson correlation feature tensor
...
def dataset_factory(cfg: DictConfig, fold: int = None) -> List[utils.DataLoader]:
    ...
    Unified dataset factory:
        ...
        - fold=0~9: enable 10-fold stratified cross validation
    Args:
        cfg: Hydra configuration object
        fold: Fold index for cross-validation (None for standard split)
    Returns:
        List of train, validation, test dataloaders
    """
    # Load full dataset
    ...
    # Mode 1: 10-Fold Stratified Cross Validation
    if fold is not None:
        ...
        # Split train+val set into train (90%) and validation (10%)
        ...
        # Update step count for learning rate scheduler
        with open_dict(cfg):
    ...
    # Mode 2: Original stratified random split
    else:
